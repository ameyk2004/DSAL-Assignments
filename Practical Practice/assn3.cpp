...
public:
    ...
    void bothChildCase(Node* curr)
    {
        Node* inorderSuccesor = curr->right;
         Node* ParentinorderSuccesor = curr;

        while(!inorderSuccesor->lthread)
        {
            ParentinorderSuccesor = inorderSuccesor;
            inorderSuccesor = inorderSuccesor->left;
        }

        curr->data = inorderSuccesor->data;
